...
MYPROJECT_ROOT="/home/$USER/atten-stereo"
if [ ! -d $MYPROJECT_ROOT ];then
	MYPROJECT_ROOT="/diskb/ccjData2/atten-stereo"
	echo "Updated : setting MYPROJECT_ROOT = ${MYPROJECT_ROOT}"
fi
...
if [ $KT2012 -eq 1 ]; then
  ...
  #TEST_LIST="lists/virtual_kitti2_wo_scene06_fixed_test.list"
	TEST_LIST="lists/virtual_kitti2_scene06_test.list"
	#TEST_LIST="lists/virtual_kitti2_scene06_test_small.list"
...
if [ "$flag" = true ]; then
	...
	do # epoch model loop
	  ...
	  # for baselines, we always use (dummy) Embed + Baseline, but disable Embed !!!
		if [ $MODEL_NAME == 'PSMNet' ]; then
			...
			TMP_MODEL_NAME="psmnet-D192-sfepo10-kt15epo400"
	  ...
	  # for baselines, we always use (dummy) Embed + Baseline, but disable Embed !!!
			...
			TMP_MODEL_NAME="ganet-sf-epo10-kt15-epoch100"
			RESUME="./checkpoints/${VKT2_ABLATION}/${TMP_MODEL_NAME}/model_epoch_$(printf "%05d" "$EPO_TEST").tar"
			EXP_NAME="${TMP_MODEL_NAME}/disp-epo-$(printf "%03d" "$EPO_TEST")"
			#already contained in the RESUME:
			RESUME_EMBEDNET=''
		...
		# for baselines, we always use (dummy) Embed + Baseline, but disable Embed !!!
			...
			TMP_MODEL_NAME="gcnetAKQ-D192-sfepo10-kt15epo400"
		...
		#----------------------
			...
			TMP_MODEL_NAME="asn-embed-k${K_WIDTH}-d${DILATION}-D${MAX_DISP}-dispnetc-sfepo20-kt15epo600-embedlossW-0.06-lr-0.001-p-eposteps-300"
		...
		#----------------------
			...
			TMP_MODEL_NAME="asn-dfn-k5-d2-D192-psm-sfepo10-kt15epo400-woEmbed"
		...
		#----------------------
			...
			TMP_MODEL_NAME="asn-pac-k${K_WIDTH}-d${DILATION}-D${MAX_DISP}-psm-sfepo10-kt15epo400-woEmbed"
			TMP_MODEL_NAME="asn-npac-k${K_WIDTH}-d2-D144-psm-sfepo10-kt15epo400-woEmbed"
			#BATCH_IN_IMAGE='true'
		...
		#----------------------
			...
			TMP_MODEL_NAME="asn-npac-k5-d2-D192-dispnetc-sfepo20-kt15epo1000-woEmbed"
			TMP_MODEL_NAME="asn-embed-k5-d2-D192-dispnetcV4-sfepo20-kt15epo500-embedlossW-0.06"
			RESUME="./checkpoints/saved/${TMP_MODEL_NAME}/ASN-PAC-DispNetC/model_epoch_$(printf "%05d" "$EPO_TEST").tar"
			EXP_NAME="${TMP_MODEL_NAME}/disp-epo-$(printf "%03d" "$EPO_TEST")"
			#already contained in the RESUME:
			RESUME_EMBEDNET=''
    ...
    #----------------------
		...
		#----------------------
			...
			TMP_MODEL_NAME="asn-npac-k5-d2-D192-ganetdeep-sfepo10-kt15epo400-woEmbed"
		...
		#----------------------
			...
			TMP_MODEL_NAME="asn-sga-k0-d2-D192-gcnetAKQ-sfepo10-kt15epo400-woEmbed"
		...
		RESULTDIR="./results/kt15-ablation/${EXP_NAME}"
	  #MODE='test'
		# debugging info
	  #let CROP_WIDTH=1280 # multiple of 64, due to SGA module
	  #let CROP_WIDTH=1248 # multiple of 48, due to SGA module
	  MODE='inferencetime'
		cd ${MYPROJECT_ROOT}
		CUDA_VISIBLE_DEVICES=$2 python3.7 -m main_attenStereoNet \
			--batchSize=${BATCHSIZE} \
		...
		if [ $KT2015 -eq 1 ] || [ $KT2012 -eq 1 ]; then
			...
			if [ -f "./bad-err.csv" ]; then
				cat "./bad-err.csv" >> "${MYPROJECT_ROOT}/results/bad-err-evalu.csv"
			fi
