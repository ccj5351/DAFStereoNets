...
""" added by CCJ """
def normalize_gray_via_mean_std(img):
    assert img.ndim == 2
...
def load_kitti2012_gray_data(file_path, current_file):
    """ load current file from the list"""
    ...
    temp_data[0] = normalize_gray_via_mean_std(left) 
    temp_data[1] = normalize_gray_via_mean_std(left)
    temp_data[2] = normalize_gray_via_mean_std(left)
    # copy right gray image to each of the three ones
    temp_data[3] = normalize_gray_via_mean_std(right) 
    temp_data[4] = normalize_gray_via_mean_std(right)
    temp_data[5] = normalize_gray_via_mean_std(right)
    temp_data[6, :, :] = disp_left
    # save for tensorboard visualization
...
class DatasetFromList(data.Dataset): 
    def __init__(self, data_path, file_list, crop_size=[256, 256], 
            training=True, kitti2012=False, kitti2015=False, shift=0, 
            is_semantic=True,
            is_kt12_gray = False
            ):
        ...
        self.is_kt12_gray = is_kt12_gray

    def __getitem__(self, index):
    #    print self.file_list[index]
        if self.kitti2012 and self.is_kt12_gray: #load kitti2012 gray dataset
            temp_data = load_kitti2012_gray_data(self.data_path, self.file_list[index])
        elif self.kitti2012 and (not self.is_kt12_gray): #load kitti2012 color dataset
