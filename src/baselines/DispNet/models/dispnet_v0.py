...
# @last modified: Fri 17 Jan 2020 03:41:45 AM EST
...
class DispNet(nn.Module):
    ...
    def __init__(self, is_corr = True, maxdisp = 40, 
        ...
        print ("[***] DispNet using maxdisp : ", maxdisp)
        if self.is_corr:
            if str(corr_func_type).lower() == 'correlation1d_map_v1':
                self.corr_func = correlation1D_map_V1(maxdisp)
            elif str(corr_func_type).lower() == 'corr1d_v2':
                self.corr_func = Corr1d_V2(D=maxdisp)
            else:
        ...
        if self.is_corr: 
            ...
            self.conv3a = downsample_conv_bn(64+self.maxdisp,256, kernel_size = 5,is_relu = self.is_relu,is_bn = self.is_bn)
        else:
            """ DispNetS """
