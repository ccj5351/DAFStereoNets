...
segmentation and/or instance segmentation) for PyTorch. Currently, we
...
from loaddata import input_preprocess
...
class MySegDataset(Dataset):
  ...
  def __init__(self,
    ...
    """ added by CCJ """
    self.data_samples_number = _DATASETS_INFORMATION[self.dataset_name].splits_to_sizes[self.split_name]
  ...
  def _parse_function(self, example_proto):
    ...
    label = None
    if self.split_name != common.TEST_SET: # if not model testing 
      label = _decode_image(
          parsed_features['image/segmentation/class/encoded'], channels=1)

    image_name = parsed_features['image/filename']
    #print ('[???] parsed image_name = {}, image height = {}, image width = {}'.format(image_name, parsed_features['image/height'], parsed_features['image/width']))
    #print ('[???] parsed_features = {}'.format(parsed_features))
    ...
    if label is not None:
      if label.get_shape().ndims == 2:
        ...
        #print '[***] expand_dims to label'
      elif label.get_shape().ndims == 3 and label.shape.dims[2] == 1:
        #print '[***] expand_dims do nothing'
        pass
      else:
    ...
    return sample # end of function _parse_function(...) 
  ...
  def _preprocess_image(self, sample):
    ...
    # Remove common.LABELS_CLASS key in the sample since it is only used to
    # derive label and not used in training and evaluation.
    # The dictionary.pop(key[, default]) method takes two parameters: 
  ...
  def get_one_shot_iterator(self):
    ...
    #print ('??? here, before batch dataset = ', dataset)
    #print ('[***] self.batch_size = {}'.format(self.batch_size))
    dataset = dataset.batch(self.batch_size).prefetch(self.batch_size)
    """ added by CCJ for debugging """
    #iter_tmp = dataset.make_initializable_iterator()
    #el = iter_tmp.get_next()
    # Launch the graph in a session.
    #with tf.Session() as sess:
    #    sess.run(iter_tmp.initializer)
    #    print('el = {}'.format(sess.run(el)))
